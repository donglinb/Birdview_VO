...
Vector4d LineGeometry::line_to_orth(Vector6d line)
{
    Vector4d orth;
...
Vector6d LineGeometry::orth_to_line(Vector4d orth)
{
    Vector6d line;
...
Vector4d LineGeometry::plk_to_orth(Vector6d plk)
{
    Vector4d orth;
...
Vector6d LineGeometry::orth_to_plk(Vector4d orth)
{
    Vector6d plk;
...
/*
 ...
 d = O.dot(cross(AO,BO))
 */
Vector4d LineGeometry::pi_from_ppp(Vector3d x1, Vector3d x2, Vector3d x3) 
{
    Vector4d pi;
...
Vector6d LineGeometry::pipi_plk( Vector4d pi1, Vector4d pi2)
{
    Vector6d plk;
...
// 获取光心到直线的垂直点
Vector3d LineGeometry::plucker_origin(Vector3d n, Vector3d v) 
{
    return v.cross(n) / v.dot(v);
}

Matrix3d LineGeometry::skew_symmetric( Vector3d v ) 
{
    Matrix3d S;
...
Vector3d LineGeometry::point_to_pose( Eigen::Matrix3d Rcw, Eigen::Vector3d tcw , Vector3d pt_w ) 
{
    return Rcw * pt_w + tcw;
}

// 从相机坐标系到世界坐标系
Vector3d LineGeometry::point_from_pose( Eigen::Matrix3d Rcw, Eigen::Vector3d tcw, Vector3d pt_c ) 
{
...
Vector6d LineGeometry::line_to_pose(Vector6d line_w, Eigen::Matrix3d Rcw, Eigen::Vector3d tcw) 
{
    Vector6d line_c;
...
Vector6d LineGeometry::line_from_pose(Vector6d line_c, Eigen::Matrix3d Rcw, Eigen::Vector3d tcw) 
{
    Eigen::Matrix3d Rwc = Rcw.transpose();
    Vector3d twc = -Rwc*tcw;
...
// 世界坐标系到相机坐标系下
Vector6d LineGeometry::plk_to_pose( Vector6d plk_w, Eigen::Matrix3d Rcw, Eigen::Vector3d tcw ) 
{
...
Vector6d LineGeometry::plk_from_pose( Vector6d plk_c, Eigen::Matrix3d Rcw, Eigen::Vector3d tcw ) 
{
